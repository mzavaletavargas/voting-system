...
function ElectionEventResults({params}: any) {
  const [candidates, setCandidates] = useState([])
  const [totalVotes, setTotalVotes] = useState(0)
  const stompClientRef: any = useRef(null);
  ...
  useEffect(() => {
    ...
    http.get(`/election-events/${params.slug}/results`)
      .then(response => {
    ...
    const client = new StompJs.Client({
     brokerURL: `${process.env.WEBSOCKET_HOSTNAME || 'ws://localhost:8081'}/gs-guide-websocket`,
     onConnect: () => {
